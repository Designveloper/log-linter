  ...
  create(context) {
    ...
    return {
      'FunctionDeclaration > BlockStatement > :first-child': (node) => {
        ...
        if (shouldFix(text, functionName)) {
          context.report({
            ...
            fix: (fixer) => {
              const fixed = fixer[action(text)](node, `
               Log.setContext({
      ...
      'FunctionExpression > BlockStatement > :first-child': (node) => {
        ...
        switch (parentNode.type) {
          ...
          case 'VariableDeclarator':
            ...
            break;
          case 'CallExpression':
            functionName = parentNode.callee.name || parentNode.callee.property.name;
            break;
          default:
      ...
      'Program > :not(ImportDeclaration)': (node) => {
        ...
        const needFix = !(new RegExp(`const LOG_CONTEXT_FILE.*${fileName.replace('.', '.*')}`, 'gm')).test(allSource);
        console.log({ needFix });
        if (needFix) {
          const shouldReplace = /const LOG_CONTEXT_FILE/.test(text);
          ...
          context.report({
            ...
            message: 'Missing context',
            fix: (fixer) => {
              const fixed = fixer[actionNeeded](node, `const LOG_CONTEXT_FILEPATH='${fileName}';\n\n`);
              return fixed;
            },
          });
          done = true;
